...
class CloudBackupManager:
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "client_id": self.drive.client_id,
        ...
        with self.database.transaction() as connection:
            connection.execute(
                ...
                (
                    ...
                    path.name,
                    str(
                        path.resolve().relative_to(self.database.config.root.resolve())
                    ),
                    checksum,
                    now,
                    now,
    ...
    async def drain(self) -> None:
        async with self._lock:
            ...
            while True:
                ...
                if not path.is_file():
                    self._finish(
                        job["id"], "FAILED", None, "File backup lokal tidak ditemukan."
                    )
                    continue
                try:
                    ...
                    if not folder_id:
                        folder_id = await asyncio.to_thread(
                            self.drive.create_backup_folder
                        )
                        with self.database.transaction() as connection:
                            _write_setting(
                                connection, "cloud_backup.folder_id", folder_id
                            )
                            _write_setting(
                                connection,
                    ...
                    self._finish(
                        job["id"], "PENDING", None, "Menunggu koneksi internet."
                    )
                    return
            ...
            try:
                if row["remote_file_id"]:
                    await asyncio.to_thread(
                        self.drive.delete_file, row["remote_file_id"]
                    )
                with self.database.transaction() as connection:
                    connection.execute(
                        "DELETE FROM cloud_backup_jobs WHERE id=?", (row["id"],)
                    )
            except ConnectionError:
                return
            except Exception:
                logger.warning(
                    "Could not apply cloud backup retention to %s", row["id"]
                )
