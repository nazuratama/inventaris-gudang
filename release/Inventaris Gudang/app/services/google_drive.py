...
GOOGLE_DESKTOP_CLIENT_ID_RE = re.compile(
...
class GoogleDriveClient:
    def __init__(self, config: AppConfig) -> None:
        self.config = config
        self._client_id = config.google_drive_client_id.strip()
        self._client_secret = config.google_drive_client_secret.strip()
    ...
    def client_id(self) -> str:
        """Return the public OAuth client identifier used by the desktop flow."""
    ...
    def configure(self, client_id: str) -> None:
        """Validate and persist a local desktop OAuth client ID."""

        normalized = client_id.strip()
        if not GOOGLE_DESKTOP_CLIENT_ID_RE.fullmatch(normalized):
            raise AppError(
                "GOOGLE_DRIVE_CLIENT_ID_INVALID",
                "Client ID Google harus berasal dari OAuth Desktop app dan berakhiran "
                ".apps.googleusercontent.com.",
                status_code=422,
            )

        changed = normalized != self._client_id
        override_path = self.config.root / "config" / "settings.json"
        try:
            ...
            raise AppError(
                "GOOGLE_DRIVE_CONFIG_WRITE_FAILED",
                "Konfigurasi lokal Google Drive belum dapat dibaca.",
                status_code=500,
            ) from exc

        payload["google_drive_client_id"] = normalized
        # Desktop clients use PKCE; a client secret is optional and should not
        # be requested from the shop operator.
        ...
        try:
            ...
            raise AppError(
                "GOOGLE_DRIVE_CONFIG_WRITE_FAILED",
                "Client ID Google belum dapat disimpan di perangkat ini.",
                status_code=500,
            ) from exc
    ...
    def delete_file(self, file_id: str) -> None:
        ...
        try:
            ...
            if exc.code == 404:
                return
            raise RuntimeError(
                f"File lama di Google Drive belum dapat dihapus ({exc.code})."
            ) from exc
    ...
    def _open_json(request: urllib.request.Request) -> dict[str, Any]:
        try:
            ...
            try:
                ...
                message = detail.get("error_description") or detail.get(
                    "error", {}
                ).get("message")
            except Exception:
                message = None
            raise RuntimeError(
                message or f"Layanan Google Drive menolak permintaan ({exc.code})."
            ) from exc
        except (urllib.error.URLError, TimeoutError) as exc:
            raise ConnectionError("Tidak dapat terhubung ke Google Drive.") from exc
        if not isinstance(payload, dict):
    ...
    def _load_token(self) -> dict[str, Any] | None:
        try:
            payload = json.loads(
                self.config.google_drive_token_path.read_text(encoding="utf-8")
            )
