...
- Google Drive uses desktop OAuth with PKCE and the narrow `drive.file` scope.
  The public Client ID and private user tokens are stored only in Git-ignored
  local files. Their file access follows the Windows account and folder ACLs.
- Update checks contact only the configured public GitHub repository. No GitHub
  token is embedded. A package is rejected without a SHA-256 digest and a valid
  file manifest.
