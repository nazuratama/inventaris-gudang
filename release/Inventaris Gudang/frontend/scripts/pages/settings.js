...
const SETTINGS_SECTIONS = [
  {
    id: "account-security",
    label: "Akun & keamanan",
    description: "Identitas dan akses lokal",
    iconName: "shield",
  },
  {
    id: "appearance",
  ...
  },
  {
    id: "data-backup",
    label: "Data & backup",
    description: "Data contoh dan Google Drive",
    iconName: "backup",
  },
  {
    id: "system-updates",
    label: "Sistem & pembaruan",
    description: "Status, retensi, dan versi",
    iconName: "server",
  },
];
...
export async function openSettingsWindow(options = {}) {
  ...
  const requestedSection = SETTINGS_SECTION_ALIASES[options.section] || options.section;
  const initialSection = SETTINGS_SECTIONS.some((entry) => entry.id === requestedSection)
    ? requestedSection
    : "account-security";
...
function renderSettingsSection(sectionId, view, context, chrome) {
  switch (sectionId) {
    case "account-security":
      return renderSettingsGroup(view, context, chrome, [
        renderAccountSection,
...
function renderSettingsGroup(view, context, chrome, renderers) {
  const grouped = settingsStack();
  const footer = { start: [], end: [] };
  const groupedChrome = {
    setFooter: ({ start = [], end = [] } = {}) => {
      footer.start.push(...start);
      footer.end.push(...end);
    },
  };

  for (const render of renderers) {
    const section = render(view, context, groupedChrome);
    if (section?.classList?.contains("settings-window-stack")) {
      grouped.append(...Array.from(section.childNodes));
    } else if (section) {
      grouped.append(section);
    }
  }
  chrome?.setFooter?.(footer);
  return grouped;
}
