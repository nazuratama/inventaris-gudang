...
async def update_settings(request: Request, payload: SettingsUpdate) -> dict[str, Any]:
    if payload.cloud_backup_enabled and not request.app.state.config.google_drive_client_id:
        raise AppError(
...
async def get_branding_image(request: Request, kind: str) -> Response:
    ...
    if not path.is_file():
        raise AppError("BRANDING_NOT_FOUND", "File branding tidak ditemukan.", status_code=404)
